...
class CUT2SEGModel_TEST(BaseModel):
    ...
    def test(self):
        ...
        self.metric_Dice = metric_Dice((self.seg_real_B>=0.5).float(), self.seg_real_B_gt.float())
        self.metric_Assd = assd((self.seg_real_B>=0.5).float().cpu().numpy(), self.seg_real_B_gt.float().cpu().numpy())
    
    def our_validdice(self):
        return self.metric_Dice.item()
    
    def our_validassd(self):
        return self.metric_Assd
    ...
    def save_2D_image(self, root):
        i = 0
        save_img = '%s'%(self.image_paths[i].split('/')[-1])
        save_path = os.path.join(root, save_img)
        image = self.seg_real_B
        image = image.squeeze(0)
        image = image.squeeze(0)
        image = (image>=0.5).float()*255
        image = image.detach().cpu().numpy()  # convert it into a numpy array
        image = image.astype(np.uint8)
        cv2.imwrite(save_path,image)

    def save_gt_image(self, root):
        i = 0
        save_img = '%s'%(self.image_paths[i].split('/')[-1])
        save_path = os.path.join(root, save_img)
        image = self.seg_real_B_gt
        image = image.squeeze(0)
        image = image.squeeze(0)
    ...
    def save_image(self, root):
        ...
        image = (np.transpose(image, (1, 2, 0)) + 1) / 2.0 * 255.0  # post-processing: tranpose and scaling
        image = image.astype(np.uint8)
        cv2.imwrite(save_path,image)
...
class Dice_coef(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        '''y_true_f = y_true.view(y_true.size(0), -1)    # flatten the output of y_true to (batch_size, width*height*depth)
        y_pred_f = y_pred.view(y_pred.size(0), -1)'''
            # flatten the output of y_pred to (batch_size, width*height*depth)
        y_true_f = y_true.view(-1)    # flatten the output of y_true to (batch_size, width*height*depth)
